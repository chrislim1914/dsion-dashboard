      ...
      if (store.state.users.responseData === 'Token has expired') {
        store.dispatch('getUserNewToken', { token: cookieMonster }).then(() => {
          if (store.state.users.responseData === 'token_invalid' || store.state.users.responseData.result === false) {
            ...
            // Expire cookies
            $cookie.delete('tka', {domain: '.dsion.io'})
            $cookie.delete('tkp', {domain: '.dsion.io'})
            $cookie.delete('tks', {domain: '.dsion.io'})
            $cookie.delete('b', {domain: '.dsion.io'})
            ...
            $cookie.set('tka', shifted[0], {expires: 1, domain: '.dsion.io'})
            $cookie.set('tkp', shifted[1], {expires: 1, domain: '.dsion.io'})
            $cookie.set('tks', shifted[2], {expires: 1, domain: '.dsion.io'})
            $cookie.set('b', surprise, {expires: 1, domain: '.dsion.io'})
          }
        })
